...
"8a,   ,aa 88 88b,   ,a8" 88       88 "8b,   ,aa 88          
 '"Ybbd8"' 88 88'YbbdP"'  88       88  '"Ybbd8"' 88          
              88                                             
              88""")
def encrypt(original_text,shift_amount):            #For Encryption Of Data
    Encrypted_Message=""
    sums=0
    original_text=original_text.lower()
    for letter in original_text:
        if letter in alphabet:
            Alpha_index=(alphabet.index(letter)+shift_amount)%len(alphabet)       #For Wrapping UP
            Encrypted_Message+=alphabet[Alpha_index]
        else:
            Encrypted_Message+=letter
    print(f"Your encoded message=> \n'{Encrypted_Message}'\nShare it secretly and also Your shift number")
def decode(original_text,shift_amount):                                             #For Decoding Funciton
    plain_text=""
